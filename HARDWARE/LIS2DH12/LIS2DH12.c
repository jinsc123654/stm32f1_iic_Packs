...
{
    Ana_I2C_E_W( LIS2DH12_ADDR,addr, data );
    return 0;
}
int32_t drv_lis2dh12_iic_read_byte(uint8_t addr, uint8_t* data)
{
    *data = Ana_I2C_E_R(LIS2DH12_ADDR,addr);
    return 0;
}
...
{
    Ana_I2C_E_W( LIS2DH12_ADDR,reg, data );
    return 1;
}
...
{
    *data = Ana_I2C_E_R(LIS2DH12_ADDR,reg);
    return 1;
}
...
{
    sample->new_angle_x = atan(1.0*(short)sample->x/(short)sqrt(pow(sample->y, 2)+pow(sample->z, 2))) * DEGREE_CAL;
    sample->new_angle_y = atan(1.0*(short)sample->y/(short)sqrt(pow(sample->x, 2)+pow(sample->z, 2))) * DEGREE_CAL;
    sample->new_angle_z = atan(1.0*(short)sample->z/(short)sqrt(pow(sample->x, 2)+pow(sample->y, 2))) * DEGREE_CAL;
...
{
    sample->old_angle_x = atan(1.0*(short)sample->x/(short)sqrt(pow(sample->y, 2)+pow(sample->z, 2))) * DEGREE_CAL;
    sample->old_angle_y = atan(1.0*(short)sample->y/(short)sqrt(pow(sample->x, 2)+pow(sample->z, 2))) * DEGREE_CAL;
    sample->old_angle_z = atan(1.0*(short)sample->z/(short)sqrt(pow(sample->x, 2)+pow(sample->y, 2))) * DEGREE_CAL;
